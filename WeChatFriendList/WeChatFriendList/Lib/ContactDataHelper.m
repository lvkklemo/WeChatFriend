...
+ (NSMutableArray *) getFriendListDataBy:(NSMutableArray *)array{
    ...
    NSArray *serializeArray = [(NSArray *)array sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {//排序
        int i;
        NSString *strA = ((ContactModel *)obj1).relatedDoctor.pinyin;
        NSString *strB = ((ContactModel *)obj2).relatedDoctor.pinyin;
    ...
    for (ContactModel *user in serializeArray) {
        char c = [user.relatedDoctor.pinyin characterAtIndex:0];
        if (!isalpha(c)) {
            [oth addObject:user];
        }
        else if (c != lastC){
